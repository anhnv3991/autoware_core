#ifndef NDT_GPU_VOXEL_GRID_HPP_
#define NDT_GPU_VOXEL_GRID_HPP_
...
class MultiVoxelGridCovariance
{
  ...
  using GpuCloudPtr = typename PointCloud<PointT>::SharedPtr;

public:
  /** \brief
   * Simple structure to hold pointers to centroid, inverse covariance of a voxel
   */
  struct Leaf
  {
  ...
  // Set input points 
  void setInput(GpuCloudPtr input);

  // set the minimum number of points in a voxel 
  // Voxels having less than this number of points are ignored in the radius search
  void setMinVoxelSize(int size);

  // Set the resolution of the voxel grid
  ...
  void radiusSearch(
    GpuCloudPtr query_cloud, 
    float radius, int max_nn,
    thrust::device_vector<int> & starting_nn_idx,
    thrust::device_vector<int> & nn_idx
  );

  int getVoxelNum() const { return voxel_num_; }

  float getMaxX() const { return }
  float getMaxY() const;
  float getMaxZ() const;
...
private:
  ...
  void scatterPointsToMultiVoxelGridCovariance();
