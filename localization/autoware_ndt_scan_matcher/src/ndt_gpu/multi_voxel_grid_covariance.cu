...
double * VoxelGrid::getCentroidList() const
{
  return centroid_;
}

double * VoxelGrid::getCovarianceList() const
{
  return covariance_;
}

double * VoxelGrid::getInverseCovarianceList() const
{
  return inverse_covariance_;
}

int * VoxelGrid::getPointsPerVoxelList() const
{
  return points_per_voxel_;
}

extern "C" __device__ int voxelId(
  float x, float y, float z, float voxel_x, float voxel_y, float voxel_z, int min_b_x, int min_b_y,
  int min_b_z, int vgrid_x, int vgrid_y, int vgrid_z)
{
...
extern "C" __global__ void initCentroidAndCovariance(
  float * x, float * y, float * z, int * starting_point_ids, int * point_ids, double * centroids,
  double * covariances, int voxel_num)
{
...
extern "C" __global__ void updateVoxelCentroid(
  double * centroid, int * points_per_voxel, int voxel_num)
{
...
extern "C" __global__ void updateVoxelCovariance(
  double * centroid, double * pt_sum, double * covariance, int * points_per_voxel, int voxel_num,
...
extern "C" __global__ void computeInverseEigenvectors(
  double * inverse_covariance, int * points_per_voxel, int voxel_num, double * eigenvectors,
  int min_points_per_voxel)
{
  int index = threadIdx.x + blockIdx.x * blockDim.x;
  int stride = blockDim.x * gridDim.x;
...
extern "C" __global__ void updateCovarianceS0(
  int * points_per_voxel, int voxel_num, double * eigenvalues, double * eigenvectors,
  int min_points_per_voxel)
{
  int index = threadIdx.x + blockIdx.x * blockDim.x;
  int stride = blockDim.x * gridDim.x;
...
extern "C" __global__ void updateCovarianceS1(
  double * covariance, double * inverse_covariance, int * points_per_voxel, int voxel_num,
  double * eigenvectors, int min_points_per_voxel, int col)
{
  ...
  for (int vid = index; vid < voxel_num; vid += stride) {
    if (points_per_voxel[vid] >= min_points_per_voxel) {
      ...
      cov(0, col) =
        eigen_vectors(0, 0) * tmp0 + eigen_vectors(0, 1) * tmp1 + eigen_vectors(0, 2) * tmp2;
      cov(1, col) =
        eigen_vectors(1, 0) * tmp0 + eigen_vectors(1, 1) * tmp1 + eigen_vectors(1, 2) * tmp2;
      cov(2, col) =
        eigen_vectors(2, 0) * tmp0 + eigen_vectors(2, 1) * tmp1 + eigen_vectors(2, 2) * tmp2;
    }
...
extern "C" __global__ void computeInverseCovariance(
  double * covariance, double * inverse_covariance, int * points_per_voxel, int voxel_num,
  int min_points_per_voxel)
{
  int index = threadIdx.x + blockIdx.x * blockDim.x;
...
__global__ void init(T * input, int size, int local_size)
{
  for (int i = threadIdx.x + blockIdx.x * blockDim.x; i < size; i += blockDim.x * gridDim.x) {
    for (int j = 0; j < local_size; j++) input[i + j * size] = 1;
  }
}
...
extern "C" __global__ void normalize(
  SymmetricEigensolver3x3 sv, int * points_per_voxel, int voxel_num, int min_points_per_voxel)
{
  for (int id = threadIdx.x + blockIdx.x * blockDim.x; id < voxel_num;
...
extern "C" __global__ void computeEigenvalues(
  SymmetricEigensolver3x3 sv, int * points_per_voxel, int voxel_num, int min_points_per_voxel)
{
  for (int id = threadIdx.x + blockIdx.x * blockDim.x; id < voxel_num;
       id += blockDim.x * gridDim.x) {
    if (points_per_voxel[id] >= min_points_per_voxel) sv.computeEigenvalues(id);
    __syncthreads();
  }
}

/* First step to compute eigenvector 0 of covariance matrices. */
extern "C" __global__ void computeEvec00(
  SymmetricEigensolver3x3 sv, int * points_per_voxel, int voxel_num, int min_points_per_voxel)
{
  for (int id = threadIdx.x + blockIdx.x * blockDim.x; id < voxel_num;
       id += blockDim.x * gridDim.x) {
    if (points_per_voxel[id] >= min_points_per_voxel) sv.computeEigenvector00(id);
    __syncthreads();
  }
}
  ...
  SymmetricEigensolver3x3 sv, int * points_per_voxel, int voxel_num, int min_points_per_voxel)
{
  for (int id = threadIdx.x + blockIdx.x * blockDim.x; id < voxel_num;
       id += blockDim.x * gridDim.x) {
    if (points_per_voxel[id] >= min_points_per_voxel) sv.computeEigenvector01(id);
    __syncthreads();
  }
}

/* First step to compute eigenvector 1 of covariance matrices. */
extern "C" __global__ void computeEvec10(
  SymmetricEigensolver3x3 sv, int * points_per_voxel, int voxel_num, int min_points_per_voxel)
{
  for (int id = threadIdx.x + blockIdx.x * blockDim.x; id < voxel_num;
       id += blockDim.x * gridDim.x) {
    if (points_per_voxel[id] >= min_points_per_voxel) sv.computeEigenvector10(id);
    __syncthreads();
  }
}

/* Second step to compute eigenvector 1 of covariance matrices. */
extern "C" __global__ void computeEvec11(
  SymmetricEigensolver3x3 sv, int * points_per_voxel, int voxel_num, int min_points_per_voxel)
{
  for (int id = threadIdx.x + blockIdx.x * blockDim.x; id < voxel_num;
       id += blockDim.x * gridDim.x) {
    if (points_per_voxel[id] >= min_points_per_voxel) sv.computeEigenvector11(id);
    __syncthreads();
  }
}

/* Compute eigenvector 2 of covariance matrices. */
extern "C" __global__ void computeEvec2(
  SymmetricEigensolver3x3 sv, int * points_per_voxel, int voxel_num, int min_points_per_voxel)
{
  for (int id = threadIdx.x + blockIdx.x * blockDim.x; id < voxel_num;
       id += blockDim.x * gridDim.x) {
    if (points_per_voxel[id] >= min_points_per_voxel) sv.computeEigenvector2(id);
    __syncthreads();
  }
}

/* Final step to compute eigenvalues. */
extern "C" __global__ void updateEval(
  SymmetricEigensolver3x3 sv, int * points_per_voxel, int voxel_num, int min_points_per_voxel)
{
  for (int id = threadIdx.x + blockIdx.x * blockDim.x; id < voxel_num;
       id += blockDim.x * gridDim.x) {
    if (points_per_voxel[id] >= min_points_per_voxel) sv.updateEigenvalues(id);
    __syncthreads();
  }
}

/* Update eigenvalues in the case covariance matrix is nearly singular. */
extern "C" __global__ void updateEval2(
  double * eigenvalues, int * points_per_voxel, int voxel_num, int min_points_per_voxel)
{
  for (int id = threadIdx.x + blockIdx.x * blockDim.x; id < voxel_num;
       id += blockDim.x * gridDim.x) {
    if (points_per_voxel[id] >= min_points_per_voxel) {
...
void VoxelGrid::computeCentroidAndCovariance()
{
  ...
  initCentroidAndCovariance<<<grid_x, block_x>>>(
    x_, y_, z_, starting_point_ids_, point_ids_, centroid_, covariance_, voxel_num_);
  ...
  checkCudaErrors(
    cudaMemcpy(pt_sum, centroid_, sizeof(double) * voxel_num_ * 3, cudaMemcpyDeviceToDevice));

  updateVoxelCentroid<<<grid_x, block_x>>>(centroid_, points_per_voxel_, voxel_num_);
  checkCudaErrors(cudaGetLastError());

  updateVoxelCovariance<<<grid_x, block_x>>>(
    centroid_, pt_sum, covariance_, points_per_voxel_, voxel_num_, min_points_per_voxel_);
  checkCudaErrors(cudaGetLastError());
  checkCudaErrors(cudaDeviceSynchronize());
  ...
  updateEval2<<<grid_x, block_x>>>(
    eigenvalues_dev, points_per_voxel_, voxel_num_, min_points_per_voxel_);
  checkCudaErrors(cudaGetLastError());

  computeInverseEigenvectors<<<grid_x, block_x>>>(
    inverse_covariance_, points_per_voxel_, voxel_num_, eigenvectors_dev, min_points_per_voxel_);
  checkCudaErrors(cudaGetLastError());

  updateCovarianceS0<<<grid_x, block_x>>>(
    points_per_voxel_, voxel_num_, eigenvalues_dev, eigenvectors_dev, min_points_per_voxel_);
  checkCudaErrors(cudaGetLastError());

  for (int i = 0; i < 3; i++) {
    updateCovarianceS1<<<grid_x, block_x>>>(
      covariance_, inverse_covariance_, points_per_voxel_, voxel_num_, eigenvectors_dev,
      min_points_per_voxel_, i);
    checkCudaErrors(cudaGetLastError());
  }
  checkCudaErrors(cudaDeviceSynchronize());

  computeInverseCovariance<<<grid_x, block_x>>>(
    covariance_, inverse_covariance_, points_per_voxel_, voxel_num_, min_points_per_voxel_);
  checkCudaErrors(cudaGetLastError());
...
void VoxelGrid::setInput(float * x, float * y, float * z, int points_num)
{
...
extern "C" __global__ void findMax(float * x, float * y, float * z, int full_size, int half_size)
{
  ...
  for (int i = index; i < half_size; i += stride) {
    x[i] =
      (i + half_size < full_size) ? ((x[i] >= x[i + half_size]) ? x[i] : x[i + half_size]) : x[i];
    y[i] =
      (i + half_size < full_size) ? ((y[i] >= y[i + half_size]) ? y[i] : y[i + half_size]) : y[i];
    z[i] =
      (i + half_size < full_size) ? ((z[i] >= z[i + half_size]) ? z[i] : z[i + half_size]) : z[i];
  }
}
...
extern "C" __global__ void findBoundariesOfCandidateVoxels(
  float * x, float * y, float * z, float radius, int points_num, float voxel_x, float voxel_y,
  float voxel_z, int max_b_x, int max_b_y, int max_b_z, int min_b_x, int min_b_y, int min_b_z,
  int * max_vid_x, int * max_vid_y, int * max_vid_z, int * min_vid_x, int * min_vid_y,
  int * min_vid_z, int * candidate_voxel_per_point)
{
...
extern "C" __global__ void collectValidPoints(
  int * valid_points_mark, int * valid_points_id, int * valid_points_location, int points_num)
{
  for (int index = threadIdx.x + blockIdx.x * blockDim.x; index < points_num;
...
extern "C" __global__ void updateCandidateVoxelIds(
  int points_num, int vgrid_x, int vgrid_y, int vgrid_z, int * max_vid_x, int * max_vid_y,
  int * max_vid_z, int * min_vid_x, int * min_vid_y, int * min_vid_z, int * starting_voxel_id,
 ...
 */
extern "C" __global__ void inspectCandidateVoxels(
  float * x, float * y, float * z, float radius, int max_nn, int points_num, double * centroid,
  int * points_per_voxel, int offset, int * starting_voxel_id, int * candidate_voxel_id,
...
extern "C" __global__ void collectValidVoxels(
  int * valid_voxels_mark, int * candidate_voxel_id, int * output, int * writing_location,
  int candidate_voxel_num)
{
  for (int index = threadIdx.x + blockIdx.x * blockDim.x; index < candidate_voxel_num;
       index += blockDim.x * gridDim.x) {
...
extern "C" __global__ void collectValidVoxelCount(
  int * input_valid_voxel_count, int * output_valid_voxel_count, int * writing_location,
  int points_num)
{
  for (int id = threadIdx.x + blockIdx.x * blockDim.x; id < points_num;
       id += blockDim.x * gridDim.x) {
...
template <typename T>
void VoxelGrid::ExclusiveScan(T * input, int ele_num, T * sum)
{
...
template <typename T>
void VoxelGrid::ExclusiveScan(T * input, int ele_num)
{
  thrust::device_ptr<T> dev_ptr(input);
...
void VoxelGrid::radiusSearch(
  float * qx, float * qy, float * qz, int points_num, float radius, int max_nn, int ** valid_points,
  int ** starting_voxel_id, int ** valid_voxel_id, int * valid_voxel_num, int * valid_points_num)
{
  ...
  findBoundariesOfCandidateVoxels<<<grid_x, block_x>>>(
    qx, qy, qz, radius, points_num, voxel_x_, voxel_y_, voxel_z_, max_b_x_, max_b_y_, max_b_z_,
    min_b_x_, min_b_y_, min_b_z_, max_vid_x, max_vid_y, max_vid_z, min_vid_x, min_vid_y, min_vid_z,
  ...
  updateCandidateVoxelIds<<<grid_x, block_x>>>(
    points_num, vgrid_x_, vgrid_y_, vgrid_z_, max_vid_x, max_vid_y, max_vid_z, min_vid_x, min_vid_y,
    min_vid_z, candidate_voxel_num_per_point, candidate_voxel_id);
  checkCudaErrors(cudaGetLastError());
  checkCudaErrors(cudaDeviceSynchronize());
  ...
  inspectCandidateVoxels<<<grid_x, block_x>>>(
    qx, qy, qz, radius, max_nn, points_num, centroid_, points_per_voxel_, voxel_num_,
    candidate_voxel_num_per_point, candidate_voxel_id, valid_voxel_mark, valid_voxel_count,
    valid_points_mark);
  checkCudaErrors(cudaGetLastError());
  checkCudaErrors(cudaDeviceSynchronize());
  ...
  checkCudaErrors(cudaMemcpy(
    valid_points_location, valid_points_mark, sizeof(int) * points_num, cudaMemcpyDeviceToDevice));
  ...
  collectValidPoints<<<grid_x, block_x>>>(
    valid_points_mark, *valid_points, valid_points_location, points_num);
  checkCudaErrors(cudaGetLastError());
  checkCudaErrors(cudaDeviceSynchronize());

  checkCudaErrors(cudaMalloc(starting_voxel_id, sizeof(int) * (*valid_points_num + 1)));

  collectValidVoxelCount<<<grid_x, block_x>>>(
    valid_voxel_count, *starting_voxel_id, valid_points_location, points_num);
  checkCudaErrors(cudaGetLastError());
  checkCudaErrors(cudaDeviceSynchronize());
  ...
  checkCudaErrors(cudaMemcpy(
    valid_voxel_location, valid_voxel_mark, sizeof(int) * total_candidate_voxel_num,
  ...
  collectValidVoxels<<<grid_x, block_x>>>(
    valid_voxel_mark, candidate_voxel_id, *valid_voxel_id, valid_voxel_location,
    total_candidate_voxel_num);
  checkCudaErrors(cudaGetLastError());
  checkCudaErrors(cudaDeviceSynchronize());
...
extern "C" __global__ void buildParent(
  double * child_centroids, int * points_per_child, int child_grid_x, int child_grid_y,
  int child_grid_z, int child_num, double * parent_centroids, int * points_per_parent,
  int parent_grid_x, int parent_grid_y, int parent_grid_z)
{
  ...
  if (idx < parent_grid_x && idy < parent_grid_y && idz < parent_grid_z) {
    ...
    MatrixDevice parent_centr(
      3, 1, parent_grid_x * parent_grid_y * parent_grid_z, parent_centroids + parent_idx);
    double pc0, pc1, pc2;
    int points_num = 0;
    double dpoints_num;
...
extern "C" __global__ void insertPointsToGrid(
  float * x, float * y, float * z, int points_num, int * points_per_voxel, int voxel_num,
  int vgrid_x, int vgrid_y, int vgrid_z, float voxel_x, float voxel_y, float voxel_z, int min_b_x,
  int min_b_y, int min_b_z)
{
  int index = threadIdx.x + blockIdx.x * blockDim.x;
  int stride = blockDim.x * gridDim.x;
  ...
  for (int i = index; i < points_num; i += stride) {
    ...
    float t_y = y[i];
    float t_z = z[i];
    int voxel_id = voxelId(
      t_x, t_y, t_z, voxel_x, voxel_y, voxel_z, min_b_x, min_b_y, min_b_z, vgrid_x, vgrid_y,
      vgrid_z);

    // Update number of points in the voxel
    int ptr_increment =
      (voxel_id < voxel_num) * voxel_id;  // if (voxel_id < voxel_num), then use voxel_id
...
extern "C" __global__ void scatterPointsToVoxels(
  float * x, float * y, float * z, int points_num, int voxel_num, float voxel_x, float voxel_y,
  float voxel_z, int min_b_x, int min_b_y, int min_b_z, int vgrid_x, int vgrid_y, int vgrid_z,
  int * writing_locations, int * point_ids)
{
  ...
  for (int i = idx; i < points_num; i += stride) {
    int voxel_id = voxelId(
      x[i], y[i], z[i], voxel_x, voxel_y, voxel_z, min_b_x, min_b_y, min_b_z, vgrid_x, vgrid_y,
      vgrid_z);
...
void VoxelGrid::scatterPointsToVoxelGrid()
{
  ...
  insertPointsToGrid<<<grid_x, block_x>>>(
    x_, y_, z_, points_num_, points_per_voxel_, voxel_num_, vgrid_x_, vgrid_y_, vgrid_z_, voxel_x_,
    voxel_y_, voxel_z_, min_b_x_, min_b_y_, min_b_z_);
  checkCudaErrors(cudaGetLastError());
  checkCudaErrors(cudaDeviceSynchronize());
  ...
  checkCudaErrors(cudaMemcpy(
    starting_point_ids_, points_per_voxel_, sizeof(int) * voxel_num_, cudaMemcpyDeviceToDevice));

  ExclusiveScan(starting_point_ids_, voxel_num_ + 1);

  checkCudaErrors(cudaMemcpy(
    writing_location, starting_point_ids_, sizeof(int) * voxel_num_, cudaMemcpyDeviceToDevice));

  checkCudaErrors(cudaMalloc(&point_ids_, sizeof(int) * points_num_));

  scatterPointsToVoxels<<<grid_x, block_x>>>(
    x_, y_, z_, points_num_, voxel_num_, voxel_x_, voxel_y_, voxel_z_, min_b_x_, min_b_y_, min_b_z_,
    vgrid_x_, vgrid_y_, vgrid_z_, writing_location, point_ids_);
  checkCudaErrors(cudaGetLastError());
  checkCudaErrors(cudaDeviceSynchronize());
...
void VoxelGrid::buildOctree()
{
  ...
  while (node_number > 8) {
    ...
    buildParent<<<grid, block>>>(
      child_centroids, points_per_child, child_grid_x, child_grid_y, child_grid_z,
      child_grid_x * child_grid_y * child_grid_z, parent_centroids, points_per_parent,
...
extern "C" __global__ void nearestOctreeNodeSearch(
  float * x, float * y, float * z, int * vid_x, int * vid_y, int * vid_z, int points_num,
  double * centroids, int * points_per_node, int vgrid_x, int vgrid_y, int vgrid_z, int node_num)
{
  int idx = threadIdx.x + blockIdx.x * blockDim.x;
  int stride = blockDim.x * gridDim.x;
...
extern "C" __global__ void nearestPointSearch(
  float * qx, float * qy, float * qz, int qpoints_num, float * rx, float * ry, float * rz,
  int rpoints_num, int * vid_x, int * vid_y, int * vid_z, int vgrid_x, int vgrid_y, int vgrid_z,
  int voxel_num, int * starting_point_id, int * point_id, double * min_distance)
{
  int idx = threadIdx.x + blockIdx.x * blockDim.x;
  int stride = blockDim.x * gridDim.x;
...
extern "C" __global__ void verifyDistances(
  int * valid_distance, double * min_distance, double max_range, int points_num)
{
  int idx = threadIdx.x + blockIdx.x * blockDim.x;
  int stride = blockDim.x * gridDim.x;
...
void VoxelGrid::nearestNeighborSearch(
  float * trans_x, float * trans_y, float * trans_z, int point_num, int * valid_distance,
  double * min_distance, float max_range)
{
  ...
  for (int i = octree_centroids_.size() - 1; i >= 0; i--) {
    ...
    nearestOctreeNodeSearch<<<grid_x, block_x>>>(
      trans_x, trans_y, trans_z, vid_x, vid_y, vid_z, point_num, centroids, points_per_node,
      vgrid_x, vgrid_y, vgrid_z, node_num);
    checkCudaErrors(cudaGetLastError());
  }

  nearestPointSearch<<<grid_x, block_x>>>(
    trans_x, trans_y, trans_z, point_num, x_, y_, z_, points_num_, vid_x, vid_y, vid_z, vgrid_x_,
    vgrid_y_, vgrid_z_, voxel_num_, starting_point_ids_, point_ids_, min_distance);
  checkCudaErrors(cudaGetLastError());
